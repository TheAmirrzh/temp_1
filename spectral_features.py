...
class SpectralFeatureExtractor:
    ...
    def _eigenvalue_correction(self, eigvals: np.ndarray, epsilon: float = 1e-6) -> np.ndarray:
        """
        ...
        """
        n = len(eigvals)
        corrected = np.copy(eigvals)
        
        # FIXED: Identify structural zeros (eigenvalues < 1e-5)
        # These encode the number of connected components
        ...
        for i in range(1, n):
            ...
            # Only correct non-zero repeated eigenvalues
            if abs(corrected[i] - corrected[i-1]) < epsilon:
                # FIXED: Geometric (multiplicative) perturbation
                # This preserves relative scale and spectral structure
                corrected[i] = corrected[i-1] * (1.0 + epsilon)
