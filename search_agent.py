...
class DiversifiedBeamSearch:
    """
    Beam search guided by model scores AND value prediction,
    with placeholders for diversity-enhancing techniques.
    """
    def __init__(self, model, dataset_instance: StepPredictionDataset, beam_width: int = 5, device: str = 'cpu'):
        self.model = model
        self.beam_width = beam_width
        self.device = device
        self.model.eval()
    ...
    def _diverse_top_k(self, applicable: List[int], heuristics: torch.Tensor, k: int) -> List[int]:
        """
        Placeholder for diverse top-k expansion (e.g., DPP).
        For now, just returns standard top-k based on heuristics.
        """
        # Ensure k is not larger than number of applicable rules
        k = min(k, len(applicable))
        if k == 0:
            return []
            
        # Get scores for applicable rules
        applicable_heuristics = heuristics[applicable]
        
        # Get top-k scores and their relative indices
        top_k_scores, top_k_relative_indices = torch.topk(applicable_heuristics, k=k)
        
        # Map relative indices back to original node indices
        applicable_tensor = torch.tensor(applicable, device=self.device)
        top_k_absolute_indices = applicable_tensor[top_k_relative_indices]
        
        return top_k_absolute_indices.tolist()

    def _cluster_prune(self, candidates: List[Tuple], k: int) -> List[Tuple]:
        """
        Placeholder for clustering-based pruning.
        For now, just prunes based on highest score.
        """
        candidates.sort(key=lambda x: x[1], reverse=True) # Sort by score
        return candidates[:k]

    @torch.no_grad()
    def search(self, initial_instance: dict, max_depth: int = 10, value_bonus: float = 0.3):
        """
        Performs beam search using model scores + value heuristic.
        """
        initial_state_data = self.simulator.get_initial_state(initial_instance)
        if initial_state_data is None:
            print("Error: Could not create initial state for beam search.")
        ...
        for depth in range(max_depth):
            ...
            # --- Batching states from all beams ---
            batch_list = [state_data for state_data, _, _ in beams]
            if not batch_list: break

            current_batch = Batch.from_data_list(batch_list).to(self.device)

            # Get model predictions (scores and *value*)
            scores_batch, _, value_batch, *_ = self.model(
                current_batch.x, current_batch.edge_index, current_batch.derived_mask,
                current_batch.step_numbers, current_batch.eigvecs, current_batch.eigvals,
                current_batch.eig_mask, current_batch.edge_attr, current_batch.batch
            )
            ...
            beam_idx = 0
            for state_data, score, path in beams:
                # Get data for this specific graph in the batch
                is_current_graph = (current_batch.batch == beam_idx)
                graph_scores = scores_batch[is_current_graph]
                graph_value = value_batch[beam_idx] # Value is per-graph
                
                # --- Compute Heuristic (Score + Value Bonus) ---
                # This heuristic guides the search
                heuristics = graph_scores + value_bonus * graph_value
                ...
                # --- Diverse Expansion ---
                # Use diverse_top_k (currently just top-k)
                top_rules_to_expand = self._diverse_top_k(
                    applicable_rules, heuristics, k=self.beam_width
                )

                for rule_node_idx in top_rules_to_expand:
                    # Get the *original* model score (not heuristic) for path scoring
                    rule_score = graph_scores[rule_node_idx].item()
                    
                    next_state_data = self.simulator.apply_rule(state_data, rule_node_idx)

                    if next_state_data:
                        new_score = score + rule_score # Path score uses original scores
                        new_path = path + [rule_node_idx]
            ...
            # --- Pruning ---
            # Use cluster_prune (currently just score-based prune)
            beams = self._cluster_prune(candidates, self.beam_width)

            if not beams:
                 break

        # Return best completed proof
        if completed_proofs:
            completed_proofs.sort(key=lambda x: x[1], reverse=True)
            return completed_proofs[0]
        elif beams: # No complete proof, return best partial
            return beams[0][2], beams[0][1]
        else:
            return None, 0.0
# [ --- END REPLACEMENT --- ]